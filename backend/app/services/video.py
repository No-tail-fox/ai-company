...
class VideoService:
    ...
    def get_workbench(self, *, tenant_id: str, user_id: str = DEMO_VIDEO_USER_ID, surface: str = "portal", limit: int = 20) -> dict:
        ...
        model_context = model_configs.model_context_for_target(
            tenant_id=tenant_id,
            target_type="builtin",
            target_key=DEFAULT_VIDEO_ROUTE_KEY,
        )
        if model_context["model_config"] is not None:
            ...
            resolved = model_configs.resolve_generation_target(
                ...
                target_type=None,
                target_key=None,
                fallback_route_key=DEFAULT_VIDEO_ROUTE_KEY,
            )
    ...
    def create_generation(self, *, tenant_id: str, payload: VideoGenerationCreate) -> dict:
        ...
        resolved = ModelConfigService(self.session).resolve_generation_target(
            ...
            fallback_route_key=payload.route_key or DEFAULT_VIDEO_ROUTE_KEY,
        )
        request_key = namespace_request_key(payload.surface, payload.request_key or f"video:{new_id()}")
